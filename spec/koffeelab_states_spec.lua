...
describe("empty & error states (Phase 10)", function()
  ...
  describe("detail screens survive a deleted target", function()
    it("recipe detail renders an inert 'not found' row", function()
      local RecipeDetail = require("ui/recipe/detail")
      local d = Nav:push(RecipeDetail:new { recipe_id = 99999 })
      d:paintTo(Screen.bb, 0, 0)
      assert.are.equal(1, #d.item_table)
      assert.is_truthy(d.item_table[1]._inert)
      assert.is_true(d:onMenuChoice(d.item_table[1]))
    end)

    it("drink detail renders an inert 'not found' row", function()
